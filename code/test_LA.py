...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    ...
    ith = 0
    metrics_for_csv = []
    metrics_for_csv.append(['case_id', 'dice', 'jc', 'hd', 'asd'])
    for image_path in loader:
        ...
        if metric_detail:
            ...
            metrics_for_csv.append([ith, single_metric[0], single_metric[1], single_metric[2], single_metric[3]])
    ...
    metrics_for_csv.append(['AVG', avg_metric[0], avg_metric[1], avg_metric[2], avg_metric[3]])
    results_file = open(test_save_path + 'result.csv', 'w', newline='')
...
def val_all_case(net, base_dir, test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24):
    ...
    for image_path in tqdm(image_list):
        ...
        for i in range(1, num_classes):
            total_metric[i-1, :] += val_cal_metric(label == i, prediction == i)
    print("Validation end")
    return total_metric / len(image_list)

def val_single_case(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
